 ...
 * Produces `null` when there is no match.
 */
...
/**
 * Returns a parser that applies the given parser zero or one times.
 * Produces the given default value when there is no match.
 */
...
/**
 * Returns a parser that applies the given parser one or more times.
 * Produces a list containing the results.
 */
...
fun <IN> oneOrMore(parser: Parser<IN, Unit>): Parser<IN, Unit> {
    return OneOrMoreProduceNothingParser(parser, null)
}
 ...
 */
@JvmName("oneOrMoreProduceNothing")
fun <IN> oneOrMore(parser: Parser<IN, Unit>, separator: Parser<IN, *>): Parser<IN, Unit> {
    return OneOrMoreProduceNothingParser(parser, discard(separator))
}

/**
 * Returns a parser that applies the given parser the given number of times.
 * Produces a list containing the results.
 */
