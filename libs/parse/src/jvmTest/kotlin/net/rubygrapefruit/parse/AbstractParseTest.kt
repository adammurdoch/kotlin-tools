...
abstract class AbstractParseTest {
    ...
    private fun <IN> PullParser<IN>.expecting(fixture: DefaultCompiledParserFixture, emptyInput: IN) {
        ...
        val failures = mutableListOf<PullParser.Failure>()
        failed.accept {
            failures.add(it)
        }
        ...
        val merged = failures.fold(Expectation.Nothing as ExpectationProvider) { a, b -> ExpectationProvider.oneOf(a, b.expected) }
    ...
    private fun ParseResult<TextFailureContext, *>.assertIsFail(offset: Int, line: Int, col: Int, failureLine: String, message: String) {
        when (this) {
            ...
            is ParseResult.Fail -> {
                ...
                assertEquals(Position(offset), context.position.position, "unexpected offset for failure $message")
    ...
    private sealed interface Inspector {
        ...
        data class IsRepeatingSingleInput(val inspector: IsSingleInput, val range: Range, val hasResult: Boolean, val expect: String?) : Inspector {
            override val expected: List<String>
    ...
    private class DefaultCompiledParserFixture : CompiledParserFixture {
        ...
        override fun expectZeroOrMoreSingleInput(hasResult: Boolean, expect: String?, config: CompiledParserFixture.() -> Unit) {
            val fixture = DefaultCompiledParserFixture()
            fixture.config()
            inspectors.add(Inspector.IsRepeatingSingleInput(fixture.inspector() as Inspector.IsSingleInput, Range.ZeroOrMoreMore, hasResult, expect))
        }
        ...
        override fun expectOneOrMoreSingleInput(hasResult: Boolean, config: CompiledParserFixture.() -> Unit) {
            val fixture = DefaultCompiledParserFixture()
            fixture.config()
            inspectors.add(Inspector.IsRepeatingSingleInput(fixture.inspector() as Inspector.IsSingleInput, Range.OneOrMoreMore, hasResult, null))
        }
    ...
    private data class Step(val advance: Int)
